...
public class CapabilityManagerAPI {
     ...
     * @see CoffeeCapabilityManager#addDefaultAttachers()
     */
    public static <T extends ICapabilityProvider, C extends CoffeeCapability<T>> void registerDynamicCoffeeAttacher(CapabilityOwner<T> owner, Capability<C> capability, Predicate<T> ownerFilter, Supplier<CoffeeCapabilityGetter<T, C>> getters) {
        TimeCore.INSTANCE.getCapabilityManager().attachDynamicCoffeeCapability(owner, capability, ownerFilter, getters);
    }
    ...
    public static <T extends ICapabilityProvider, C> void registerDynamicAttacher(CapabilityOwner<T> owner, Capability<C> capability, Predicate<T> ownerFilter, Supplier<CoffeeCapabilityGetter<T, C>> getters) {
        TimeCore.INSTANCE.getCapabilityManager().attachDynamicCapability(owner, capability, ownerFilter, getters);
    }
     ...
     * @see CoffeeCapabilityManager#addDefaultAttachers()
     */
    public static <T extends ICapabilityProvider, C extends CoffeeCapability<T>> void registerStaticCoffeeAttacher(CapabilityOwner<T> owner, Capability<C> capability, Predicate<T> ownerFilter, Function<T, C> capFactory) {
        TimeCore.INSTANCE.getCapabilityManager().attachStaticCoffeeCapability(owner, capability, ownerFilter, capFactory);
    }
    ...
    public static <T extends ICapabilityProvider, C> void registerStaticAttacher(CapabilityOwner<T> owner, Capability<C> capability, Predicate<T> ownerFilter, Function<T, C> capFactory) {
        TimeCore.INSTANCE.getCapabilityManager().attachStaticCapability(owner, capability, ownerFilter, capFactory);
    }
     ...
     */
    public static <T extends INBT> void makePlayerCapTransferOnClone(Function<PlayerEntity, INBTSerializable<T>> serializableCapExtractor) {
        TimeCore.INSTANCE.getCapabilityManager().enableKeepingPlayerCapability(serializableCapExtractor);
    }

    /**
     * Enables automatic player capability syncing on such events as:
     * <ol>
     *  <li>{@link EntityJoinWorldEvent}</li>
     ...
     */
    public static void makePlayerCapSyncOnJoin(Consumer<PlayerEntity> syncFunction) {
        TimeCore.INSTANCE.getCapabilityManager().enableSyncingPlayerCapabilityOnJoin(syncFunction);
    }
